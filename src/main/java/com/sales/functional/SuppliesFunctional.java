...
public class SuppliesFunctional {
    ...
    public static void loadMenu(){
        ...
        switch(op){
            ...
            case "7":
                ...
                break;
            case "8":
                System.out.println("Agrupación de productos por etiquetas");
                groupByTags();
                break;
            case "9":
                System.out.println("Cuantos hombres y mujeres usaron cupón");
                couponMenAndWomen();
                break;
            case "10":
    ...
    public static void TotalBySale (){
        Consumer<Sale> saleConsumer = sale -> System.out.println("El cliente " + sale.getCustomer() + "pagó " + sale.getTotal());
        Function<List<Product>, Double> totalBySale = products -> products.stream().mapToDouble(Product::getPrice).sum();
        sales.forEach(sale -> {
            sale.setTotal(totalBySale.apply(sale.getItems()));
            saleConsumer.accept(sale);
        });
    }
    public static void salesByWomenInStore (){
        System.out.println("Las mujeres que compraron en tienda fisica fueron -> " +
                sales.stream().filter(sale -> sale.getPurchasedMethod().equals("In store")).
                        filter(sale -> sale.getCustomer().getGender().equals("F"))
                        .collect(Collectors.toList()).stream().count());
        };

    public static void groupByTags() {

        Consumer<List<Product>> saleConsumer = products -> products.stream().collect(Collectors.groupingBy(Product::getTags, Collectors.toSet()));

    }

    public static void couponMenAndWomen() {
        Predicate<Sale> couponUsageWomen = sale -> sale.getCouponUsed().booleanValue()  && sale.getCustomer().getGender().equalsIgnoreCase("F");
        Predicate<Sale> couponUsageMen = sale -> sale.getCouponUsed().booleanValue() && sale.getCustomer().getGender().equalsIgnoreCase("M");
        System.out.println("El numero de mujeres que uso cupones fue -> " + sales.stream().filter(couponUsageWomen).count());
        System.out.println("El numero de hombres que uso cupones fue -> " + sales.stream().filter(couponUsageMen).count());


    }

    public static void minAndMaxSale (){
            Function<List<Product>, Double> itemsPriceFunction = products -> products.stream().mapToDouble(Product::getPrice).sum();
            sales.forEach(sale -> sale.setTotal(itemsPriceFunction.apply(sale.getItems())));
